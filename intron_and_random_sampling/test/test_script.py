...
from bingo.symbolic_regression.agraph.intron_mutation import AGraphMutation
...
def execute_generational_steps():
    ...
    evaluator = Evaluation(bff, redundant=True, multiprocess=10)

    selection_phase=BayesCrowding()
    ea = GeneralizedCrowdingEA(evaluator, crossover,
                      mutation, 0.4, 0.4, selection_phase)
    #include this for population based crossover
    variation = VarAndAll(crossover, mutation, 
                            crossover_probability, mutation_probability)
    ea.variation = variation
